...
function onPage() {
  ...
  API_GOODS.goodsList(params)
    .then((res: any) => {
      ...
      list.value = unref(currentPage) === 1 ? records : unref(list).concat(records);
      listLoading.value = false;
      listFinished.value = list.value.length >= unref(total);
    })
    .catch((error) => {
      console.error(error);
      listLoading.value = false;
              ...
              <div class="list-item-price">
                <div class="price">
                  ...
                  <div v-if="item.originalPrice > 0" class="price-origin">
                    ...
                    <span class="price-origin-integer">{{ item.price }}</span>
                  </div>
                </div>
