    ...
    >
      ...
      <div v-for="item in list" :key="item.id" class="list-item van-hairline--bottom">
        <div class="list-item-hd">
          ...
          <div class="list-item-txt">订单号:{{ item.order_id}}</div>
          <div class="list-item-txt">订单金额:{{ item.out_history }}</div>
          <div class="list-item-txt">消费时间:{{ moment(item.created_time).format('YYYY-MM-DD HH:mm:ss') }}</div>
        </div>
...
export default {
  ...
  methods: {
    ...
    onPage() {
      ...
      API_USER.myWalletConsumeApi(useUserStore().userInfo.id, params)
        .then((res) => {
